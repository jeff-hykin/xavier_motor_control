    ...
    with open(args.input_file, 'r') as in_file:
        ...
        with open(args.output_file, 'w') as out_file:
            for line in lines[-11:-2]:
                if 'Program:' in line:
                    words = line.split()
                    out_file.write(f'{args.identifier}_program_size [{words[1]}{words[2]},{words[6][1:]}]\n')
                if 'Data:' in line:
                    words = line.split()
                    out_file.write(f'{args.identifier}_data_usage [{words[1]}{words[2]},{words[6][1:]}]\n')
                if 'Heap:' in line:
                    words = line.split()
                    out_file.write(f'{args.identifier}_heap_usage [{words[1]}{words[2]},{round(100 - float(words[6][1:5]), 2)}%]\n')
