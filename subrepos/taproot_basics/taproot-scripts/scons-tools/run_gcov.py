...
def run_gcov(env, source, include_taproot_in_report, files_to_exclude_from_coverage, alias="run_gcov"):
    def run_gcov_action(target, source, env):
        ...
        try:
            subprocess.run(['lcov', '-c', '-d', '.', '-o', COVERAGE_INFO_FILE, '--no-external', '--ignore-errors', 'gcov,mismatch,source'], check=True, stdout=subprocess.DEVNULL)

            files_to_include = glob.glob(os.path.abspath('src/**/*[.cpp|.hpp]'), recursive=True)

            if include_taproot_in_report:
            ...
            lcov_only_view_src_cmd = ['lcov', '-e', COVERAGE_INFO_FILE, '--ignore-errors', 'unused']
